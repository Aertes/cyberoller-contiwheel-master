  ...
  <div>
        ...
        <div class="form-horizontal row" @keyup.enter="subSearchList({page: 1})">
          <div class="col-xs-12 col-sm-6 col-md-3 search">
            <div class="form-group">
              <label class="col-sm-4 control-label nowrap">Material Name:</label>
              <div class="col-sm-8">
    ...
    <a-window>
      <div v-if="nowPage === 'edit'">
        <edit @close="close()" @reload="subSearchList()" :material="tableInfo.select[0]"></edit>
      </div>
    </a-window>
  </div>
</template>
<script>
  ...
  import add from './materialAdd.vue'
  import edit from './materialEdit.vue'
  import { userStatus } from '../../../helper/Status'
  import messageBox from '../../../components/msgbox'
  const menu = {
    '71c1532212f211e790656c92bf3149c7': 'Promotion',
    'd6a394fe12f211e790656c92bf3149c7': 'Retail/POSM'
  }
  export default {
    components: {
      add,
    ...
    methods: {
      enable () {
        getData(xhrUrls.ENABLE_MATERIAL_INFO, {
      ...
      disable () {
        getData(xhrUrls.DISABLE_MATERIAL_INFO, {
          id: this.tableInfo.select[0].id
        }).then(res => {
          console.log(res)
          if (res.data.code === 200) {
      ...
      deleteInfo () {
        getData(xhrUrls.DELETE_MATERIAL_INFO, {
          id: this.tableInfo.select[0].id
        }).then(res => {
          console.log(res)
          if (res.data.code === 200) {
