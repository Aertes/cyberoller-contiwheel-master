  ...
  <div>
        ...
        <div class="form-horizontal row" @keyup.enter="subSearchList({page: 1})">
          <div class="col-xs-12 col-sm-6 col-md-3 search">
            <div class="form-group">
              <label class="col-sm-4 control-label nowrap">Product Name:</label>
              <div class="col-sm-8">
    ...
    <a-window>
      <div v-if="nowPage === 'edit'">
        <edit @close="close()" @reload="subSearchList()" :product="tableInfo.select[0]"></edit>
      </div>
    </a-window>
  </div>
</template>
<script>
  ...
  import add from './productAdd.vue'
  import edit from './productEdit.vue'
  ...
  export default {
    ...
    methods: {
      enable () {
        getData(xhrUrls.ENABLE_PRODUCT_INFO, {
      ...
      disable () {
        getData(xhrUrls.DISABLE_PRODUCT_INFO, {
          id: this.tableInfo.select[0].id
        }).then(res => {
          console.log(res)
          if (res.data.code === 200) {
      ...
      deleteInfo () {
        getData(xhrUrls.DELETE_PRODUCT_INFO, {
          id: this.tableInfo.select[0].id
        }).then(res => {
          console.log(res)
          if (res.data.code === 200) {
